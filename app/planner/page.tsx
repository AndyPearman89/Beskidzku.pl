...
export default function PlannerPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) setStops(JSON.parse(stored) as PlanStop[]);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  // Debounced search
  useEffect(() => {
    const timer = setTimeout(() => void searchListings(searchQuery), 350);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <p className="text-xs font-semibold text-[var(--color-muted)] uppercase tracking-wide">Planner Beskidów</p>
        <h1 className="text-3xl font-bold mt-1">Zaplanuj wycieczkę</h1>
        <p className="text-[var(--color-muted)] mt-2 text-sm">
          Dodaj miejsca do planu, sprawdź trasę i wyeksportuj do Google Maps.
        </p>
      </div>
